...
function callPromiseWithParam() {
  ...
  deleayPromiseWithParam().then((message: string) => {
...
function callPromiseWithInterface() {
  console.log("Call promise deleayPromiseWithParam");
  deleayPromiseWithInterface().then((message: IpromiseMessage) => {
    console.log(
      `Return value of Pomise.then(): message:${message.massage}, id:${message.id}`
    );
  });
}
